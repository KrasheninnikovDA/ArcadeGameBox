/// класс - механика. реализует функционал по переключению направления движения машины в состоянии CarMoveState
/// при достчижении крайней точки направления текущего движения инвокает EndPathMover и меняет направление на противоположное
///_distanceCheckTimer нужен чтобы делать задержку в проверке достижения точки, для корректного переключения направления
using UnityEngine;
